...
public:
    ...
    symbolInfo(symbolInfo* SymbolInfo){
        ...
        stackOffset = SymbolInfo->stackOffset;
...
public:
    ...
    */
    ...
    bool insertIntoSymbolTable(symbolInfo symbolInf){
        ...
        if(tableId != 1 && symbolInf.stackOffset == 0){
            if(symbolInf.isArray()){
                newSymbolInf->setStackOffset(stackOffset-2);
                stackOffset -= 2*symbolInf.getSize();
            }
            else{
                stackOffset -= 2;
                newSymbolInf->setStackOffset(stackOffset);
            }
        }
        int incr = 0;
